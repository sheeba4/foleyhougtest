...
<div id="content" role="main">
	...
	<?php if ( have_posts() ) : ?>
		...
		<h1 class="page-title">
			<?php if ( is_day() ) : ?>
				<?php printf( __( 'Daily Archives: %s', 'minify' ), '<span>' . get_the_date() . '</span>' ); ?>
			<?php elseif ( is_month() ) : ?>
				<?php printf( __( 'Monthly Archives: %s', 'minify' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'minify' ) ) . '</span>' ); ?>
			<?php elseif ( is_year() ) : ?>
				<?php printf( __( 'Yearly Archives: %s', 'minify' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'minify' ) ) . '</span>' ); ?>
			<?php else : ?>
				<?php _e( 'Blog Archives', 'minify' ); ?>
			<?php endif; ?>
		</h1>
