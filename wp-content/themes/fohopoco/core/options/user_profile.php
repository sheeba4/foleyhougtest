...
class User_Profile {
	...
	function admin_scripts() {
		wp_enqueue_media();
		wp_register_script( 'foley_user', get_template_directory_uri() . '/_ui/js/user_profile.admin.js', array(
			'jquery',
			'media-upload',
			'thickbox'
		), '1.0.3' );
	...
	public function render_user_form( $user ) { ?>
		...
		<table class="form-table">
			<tr>
				...
				<td><img src="<?php echo esc_url( $profile_image ); ?>"/>
				<td/>
				<td>
					<input type="text" class="text" size="60" name="profile_image" id="profile_image"
						   value="<?php echo esc_url( $profile_image ); ?>">
					<input type="button" id="profile_image-button" class="button"
						   value="<?php _e( 'Choose or Upload a file' ); ?>">
			...
			<tr>
				...
				<td>
					<input type="checkbox" class="checkbox" name="hide_sidebar" id="hide_sidebar"
						   value="hide" <?php checked( $hide_sidebar, 'hide' ); ?>>
